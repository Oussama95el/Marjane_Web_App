...
<div class="h-screen w-full flex overflow-hidden select-none">
    ...
    <main class="my-1 pt-2 pb-2 px-10 flex-1 bg-gray-200 dark:bg-black rounded-l-lg
        ...
        <nav class="flex items-center justify-evenly bg-white text-center">
            <div class="w-full">
                <a href="/saper-admin/manager" class="px-5 py-2.5 w-full relative group overflow-hidden font-medium bg-white text-blue inline-block">
                    ...
                    <span class="relative group-hover:text-blue">Manager List</span>
                </a>
            </div>
            <div class="w-full">
        ...
        <div class="w-full py-4 px-8 rounded bg-white">
            <form method="post" action="/super-admin/manager-creat">
                <h2 class="text-grey font-bold text-lg mb-3">Manager Form</h2>
                ...
                <div class="relative z-0 mb-6 w-full group">
                    <%--                branch Select and loop through attribute categories using <c:foreach> --%>
                    <select name="branch" id="branch"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                        <option disabled selected>Select City</option>
                        <c:forEach items="${branches}" var="branch">
                            <option class="text-sm text-gray-500 font-medium" value="${branch.getbId()}"><c:out
                                    value="${branch.getbCity().getcNom()}"/></option>
                        </c:forEach>
                    </select>
                    <label for="branch"
                           class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Banch
                        <B>*</B></label>
                </div>
                <button type="submit"
